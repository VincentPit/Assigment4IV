...
function AirportBubble(props){
    ...
    const createBubbles = (cities) => {
        // Sort the cities ascendingly by Count
        cities.sort((a, b) => a.Count - b.Count);

        // scale for the radius of bubbles
        ...
        return cities.map((d, idx) => {
            const isTopHub = idx >= cities.length - 5; // Last 5 cities are top hubs
            return (
    ...
    return (
        <g>
            {cities.map((city, idx) => {
                const isTopHub = idx >= cities.length - 5; // Top 5 hubs
                return (
